...
int main() {
    ...
    while (1) {
        ...
        recvfrom(serverASock, buffer, sizeof(buffer), 0, NULL, NULL);
        buffer[sizeof(buffer)] = '\0';
        std::cout<<"Server B has received a request for "<<buffer<<std::endl;
        std::stringstream ss;
        ss << "There are " << dataMap[buffer].size() << " distinct students in " << buffer << " department. Their IDs are ";
        std::string output = ss.str();
        std::cout<<"Server B found "<<dataMap[buffer].size()<<" distinct students for "<< buffer<< ":" <<std::to_string(dataMap[buffer][0]);
